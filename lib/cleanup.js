...
async function deleteDeveloperPermanently(uid) {
  const result = { authDeleted: false, devDocDeleted: false, appsDeleted: 0, authError: null };
  ...
  try {
    if (!auth) {
      throw new Error('Firebase Auth is not initialized. Check FIREBASE_SERVICE_ACCOUNT environment variable.');
    }

    // First check if user exists in Auth
    try {
      const userRecord = await auth.getUser(uid);
      console.log(`[Cleanup] Found Auth user: ${userRecord.email || uid}`);
    } catch (lookupErr) {
      if (lookupErr.code === 'auth/user-not-found') {
        console.log(`[Cleanup] User ${uid} already does not exist in Auth.`);
        result.authDeleted = true;
        result.authError = 'already-deleted';
      } else {
        throw lookupErr;
      }
    }

    // Only attempt delete if user was found
    ...
    const errorCode = authErr.code || authErr.errorInfo?.code || 'unknown';
    const errorMsg = authErr.message || 'Unknown error';
    result.authError = `${errorCode}: ${errorMsg}`;
    console.error(`[Cleanup] ❌ Auth delete FAILED for ${uid}:`, errorCode, errorMsg);
